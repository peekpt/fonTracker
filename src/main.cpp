...
void setup(){
        ...
        if (_firstBoot_ != 12345) { // save first boot values
          _firstBoot_  = 12345;
...
/*** PROGRAM MODE WIFI->TRACKER  IP->192.168.1.1
*         Pin 0 must have a pull up resistor (10k) and a switch to pulldown.
*         if PIN 0 is GND in 3 seconds after boot, then enter program mode
*         note Pin 0 must be high on reboot , then click to enter program mode
*/
        int tout = 100;
        while (tout != 0) {
...
// blinks  4x for error 10x for success
        ...
        ESP.deepSleep(_interval_ * 60 * 1000000);
}
